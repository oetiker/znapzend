...
sub dataSetExists {
    ...
    my @ssh = $self->$buildRemote($remote, [@{$self->priv}, qw(zfs list -H -o name -t filesystem), $dataSet]);
...
sub listDataSets {
    ...
    my @ssh = $self->$buildRemote($remote, [@{$self->priv}, qw(zfs list -H -o name -t filesystem)]);
...
sub listSubDataSets {
    ...
    ($remote, $dataSet) = $splitHostDataSet->($dataSet);
    my @ssh = $self->$buildRemote($remote, [@{$self->priv}, qw(zfs list -H -r -o name -t filesystem), $dataSet]);
