...
=item B<--dst-concurrency>

Enable destination sends in parallel for one backup set. When the option is
specified without a value, all configured destinations are processed in
parallel.

If a value is provided, it must be an integer >= 1 and limits the maximum
number of destination streams. Pass B<--dst-concurrency=off> to turn parallelism
back off (restoring serial processing) for a set that previously enabled it.

Destination parallelism is strictly opt-in. Existing backup sets that do not
define I<destination_concurrency_enabled> -- which includes every configuration
created before this feature existed -- keep the long-standing behavior of
processing destinations serially, one at a time. Upgrading therefore never
changes replication behavior on its own. New backup sets created without
B<--dst-concurrency> also default to serial destination processing and store no
concurrency property at all.

This per-backup-set control is distinct from the daemon-wide
B<--maxConcurrentSends> / B<--serialize> options of B<znapzend>, which limit how
many backup sets send at the same time. The two limits are not combined: with
per-destination concurrency enabled, a single backup set may run several
destination streams in parallel even under B<--serialize>, so the effective
number of concurrent I<zfs send> streams can exceed B<--maxConcurrentSends>
alone. Size both together.
...
create a multi-destination backup with bounded parallel sends

    znapzendzetup create --recursive --dst-concurrency=2 \
