...
AND lists properties for snapshots by default when recursing (also
the one in Solaris 10 does not), so that there is too much data to
process while searching for backup plans.
...
hundreds of times, so you would benefit from requesting this feature.
