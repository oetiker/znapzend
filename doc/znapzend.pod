...
See also B<--inherited> for interaction with that option.
...
At this time, the B<--inherited> is effectively exclusive vs. B<--recursive>
mode (and preferred if both are specified) if the requested dataset has some
(local or inherited) backup plan. If a dataset without a backup plan is passed,
the script should ignore the B<--inherited> option and just recurse until it
finds some dataset(s) with a proper locally defined backup plan.
