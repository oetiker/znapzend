...
For backward compatibility, existing backup sets that do not define
I<dst_concurrency_enabled> keep the legacy behavior (all destinations in
parallel). New backup sets created without B<--dst-concurrency> default to
serial destination processing.
