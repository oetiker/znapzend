...
znapzend with a copy of perl as old as 0.10.1:

```sh
wget https://github.com/oetiker/znapzend/releases/download/v0.10.1/znapzend-prebuilt-0.10.1.tar.gz
tar zxvf znapzend-prebuilt-0.10.1.tar.gz
cp -rp znapzend-prebuilt-0.10.1 /opt/znapzend-0.10.1
```
...
```sh
wget https://github.com/oetiker/znapzend/releases/download/v0.10.1/znapzend-0.10.1.tar.gz
tar zxvf znapzend-0.10.1.tar.gz
cd znapzend-0.10.1
./configure --prefix=/opt/znapzend-0.10.1
```
if configure complains about missing perl modules, run

```sh
./setup/build-thirdparty.sh /opt/znapzend-0.10.1/thirdparty
```
...
```sh
/opt/znapzend-0.10.1/bin/znapzend --noaction --debug
```

If you don't want to wait for the scheduler to actually schedule work, you can also force immediate action by calling

```sh
/opt/znapzend-0.10.1/bin/znapzend --noaction --debug --runonce=<src_dataset>
``` 

then when you are happy with what you got, start it in daemon mode.

```sh
/opt/znapzend-0.10.1/bin/znapzend --daemonize
```
